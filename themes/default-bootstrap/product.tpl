		...
		<!-- end Details product infos -->
						...
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="add_to_cart" class="buttons_bottom_block no-print">
								<button type="submit" name="Submit" class="exclusive">
									<span>{if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize'}{else}{l s='Add to shopping bag'}{/if}</span>
								</button>
							</p>
						</div>
			...
			<!-- Nav product next or prev -->
				<div class="product-navigation clearfix">
					{if $prev_product}
						<a title="{$prev_product.name}" class="btn btn-default" href="{$link->getProductLink($prev_product.id_product, $prev_product.link_rewrite)}"><span><i class="icon-chevron-left"></i> {l s='Previous Product'}</span></a>
					{/if}
					{if $next_product}
						<a title="{$next_product.name}" class="btn btn-default" style="float:right"href="{$link->getProductLink($next_product.id_product, $next_product.link_rewrite)}"><span>{l s='Next Product'} <i class="icon-chevron-right"></i></span></a>
					{/if}
				</div>
			<!-- Nav product next or prev -->			
